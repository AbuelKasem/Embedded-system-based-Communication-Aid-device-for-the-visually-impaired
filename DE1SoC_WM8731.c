...
signed int WM8731_initialise ( unsigned int base_address ) {
    ...
    if (status != HPS_I2C_SUCCESS) return status;
    status = HPS_I2C_write16b(0, 0x1A, (WM8731_I2C_LEFTOUTCNTRL <<9) | 0x7F); //-24dB Volume. Unmute.
    if (status != HPS_I2C_SUCCESS) return status;
    status = HPS_I2C_write16b(0, 0x1A, (WM8731_I2C_RIGHTOUTCNTRL<<9) | 0x7F); //-24dB Volume. Unmute.
    if (status != HPS_I2C_SUCCESS) return status;
